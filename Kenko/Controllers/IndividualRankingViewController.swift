...
class IndividualRankingViewController: UIViewController {
    
    @IBOutlet weak var intervalLabel: UILabel!
    private var rankingInterval: RankingInterval = .daily
    ...
    @IBAction func changeRankingPeriod(_ sender: UISegmentedControl) {
        ...
        case 0: rankingInterval = .monthly
        case 1: rankingInterval = .weekly
        case 2: rankingInterval = .daily
        default: break
        }
        reloadRankingData()
    }
    ...
    private func reloadRankingData() {
        ...
        Network.getUserIndividualRanking(interval: rankingInterval, success: { userPersonalRankingInformation in
        ...
        Network.getIndividualRanking(interval: rankingInterval, success: { personalRankingInformation in
            self.rankingList = []
    ...
    private func updatePeriodLabel(from: String, to: String) {
        switch rankingInterval {
        case .daily:
            periodLabel.text = to.decodedUTCDateDate.encodedUTCDay
        case .weekly, .monthly:
...
extension IndividualRankingViewController: UITableViewDataSource, UITableViewDelegate {
